...
リアルタイム処理を実装することで、これまで実施できなかった業務ができるようになるなど、大きなメリットもありますので、是非最後までお読みいただければ嬉しく感じます。また本章ではPower Automate for desktopで作成したサンプルフローも掲載致します。その他のRPA製品でも同様のフローは作れますので、よかったら参考にしてください。
...
こういったケースにおいて、RPAによるリアルタイム処理が活きてきます。必要な時にシナリオを動かすのではなく、リアルタイム処理で常にシナリオが動き続け、必要な条件が揃った際に必要な処理をさせることが可能となります。
...
//image[05_004][Power Automate for desktopによる無限ループシナリオ]
...
そのためには、実績値を電子化する必要があります。筆者の例だと、データを入力するだけの簡単なフォームを作り、CSVファイルに出力するようにしました。
今なら、Microsoft 365のFormsを使ったり、チャットツールから入力し、Power AutomateでCSVファイルを出力する、なんてやりかたもありそうですね。
...
併せて処理済みのファイルはbkupフォルダに移動させます。これは同じファイルで二重登録を避ける目的と、実際に登録したデータのエビデンスを残す意味があります。またログファイルも出力しています。ログファイルについては後ほど説明致します。
...
前項は在庫間移動でしたが、ここでは完成品入力になります。製造業においては、1つの機種を生産するのに、加工指示書と呼ばれる書類が発行されます。各工程は、この加工指示書に基づいて実際の生産を実施します。
...
今回はメールとしましたが、Teams等のチャットツールに送る方が、より早く気づいてもらえるメリットがあります。
...
また、登録が目的のフローであれば、登録がどこまで済んだのかの確認も必要となります。実際の登録状況と、監視フォルダやbkupフォルダ、またログファイルの内容を見比べ、どこから処理を再開させれば良いのかを判断する必要もあります。

== リアルタイム処理の今後の展望

RPAにおいては、生産性向上、品質向上の2点に注目が集められていましたが、このようにRPAを活用することで、人手ではできなかったことが実現でき、あるべき姿に近づくことができます。
全てのシステムがクラウド化されれば、クラウドフローで事足りることかもしれませんが、まだまだオンプレミスの仕組みも多いですし、クラウドからオンプレミス回帰の流れもできつつあります。また、AIによる自動化等もRPAには追い風だと思っています。

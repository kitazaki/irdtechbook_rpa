 ...
 * 業務での異常発生時の対策
 ** 入力とするファイルのデータ品質が悪く、業務遂行できない、有事の際の対策を記入します。例えば、今は実行しなくても大丈夫なのか、先の業務手順書をもとに、手作業で遂行するのかなどです。
 ** ※これはＲＰＡを改修、データを綺麗にするための猶予時間の確認のためであり、この対策があるからＲＰＡの品質を下げてもよいというものではありません。
...
0で始まる数字のデータを含んだＣＳＶファイルをＥＸＣＥＬで開くと、先頭の0はなくなる事は認識していたようですが、人の作業でこなしていたそうです。もちろんその時は、ＣＳＶファイルを入力にしたＲＰＡに改良しました。
...
とあるお客様の開発現場で、Ａロボットで使用しているプラットフォームを別のプラットフォームへと変更する改修を依頼されました。改修対象となるＡロボットを共有され、中身を解読し始め、改修概要をまとめた資料を作成し、現場担当者様との打ち合わせに臨んだのですが、開口一番、「Ａロボットは現在運用していない」と。その場でＢロボットの改修に当たる事になったのですが、共有されたＢロボットは、プラットフォーム変更以前に業務ルール改編による改修が進行中でした。２週間ほど、無意味な作業をしていたのでした。

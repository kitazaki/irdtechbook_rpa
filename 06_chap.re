...
SMSのE2E監視を実現するために、SMSの送信・受信をシミュレートする方法を検討しネットで調べたところ、Rasberry Pi@<fn>{fn-0601}とUSB接続のLTEドングルを使ってSMSを送受信を試しているネット記事を見つけました。
Rasberry Piとは、教育目的に製造された小型コンピュータで、現在普及が進んでおり教育分野以外も様々な分野で利用されています。

また、USB接続のLTEドングルは、外出した際にWifi環境がない場合に利用するUSB型のモデムの事を指します。Rasberry PiからATコマンドと呼ばれるモデムを操作するコマンドをシリアル接続でドングルに流し込む事により、通話やインターネット接続やSMSの送受信などの携帯端末の操作を実行する事ができます。今回は、ATコマンドを使ったSMSの送受信の処理をスクリプトとして実装する事によって、SMS送受信シミュレータを実現しました。詳しくは実装パートで説明します。
...
実装に当たっては社内のセキュリティ部門が提供するセキュリティガイドラインを満たすように実装する必要があります。また、実装後に脆弱性診断を受け、診断結果が問題なければ実装したものを運用開始する事ができます。また、運用を開始してからも年次で脆弱性診断を実施する必要と月次でセキュリティパッチの適用調査をする必要があります。
...
Web APIはGCP上のGoogle App Engine(GAE)@<fn>{fn-0604}とCloud SQL@<fn>{fn-0605}を使って実装しました。運用の負荷をかけずに、実装は柔軟にしたいという考えでPaaSを利用して実装しました。GAEは認証APIと登録APIと結果取得APIを提供しています。また、Cloud SQLにはシミュレータで実行したSMSの配信結果情報が登録APIで保存されています。インターネット上に公開しているため、以下のようなセキュリティの対策を行っています。
...
結果取得API、認証APIを実行するためには、認証APIで取得した認証キーを必要とするように実装しています。また、そのキーの有効期限については非常に短い期間に設定しています。
...
シミュレータはRasberry Piとドングルの費用で1台大体2万円ぐらいかかります。半導体不足の影響でRasberry Piの値段が上がっているようなので、今はもう少しかかるかもしれません。SIMの初期契約料については、MVNOのソラコムさんが提供している特定地域向け IoT SIMを契約していて、初期費用はDCMのSIMで3300円、AUのSIMで1650円でした。
...
今回、Rasberry Piを利用したSMSサービスのE2E監視を実装した経緯、システム構成、システムの実装、運用についてご紹介させて頂きました。

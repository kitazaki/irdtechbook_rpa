...
Amazonアカウントと連携して自動注文してくれたり、EメールやLINE(ID連携が必要)で通知されるので非常に便利ですが、注文先を変更したり、消費データだけを利用(分析)したり、他の機能と連携させたい場合があります。
...
在庫の消費傾向に加えて、Wi-Fi接続状況と電池の消費傾向も把握できるようにしました。在庫が無くなる時期を予測したり、使い過ぎを警告することもできそうです。
